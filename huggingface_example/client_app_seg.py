...
class IMDBClient(NumPyClient):
    def __init__(self, model_name, trainloader, testloader, local_epochs, timeout, is_demo) -> None:
    ...
    def fit(self, parameters, config):        # 👈 return type #config
        ...
        ret_vec = get_params(self.net)
        print(f"[Client] Training time: {train_time:.2f} seconds")
        avg_loss = float(sum(train_losses) / len(train_losses))
...
def client_fn(context: Context) -> Client:
    ...
    return IMDBClient(model_name, trainloader, testloader, local_epochs, is_demo, timeout).to_client()
